...
const ModuleSidebarItem = ({
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-[#222832] bg-[#1E2329] shadow-lg shadow-black/30">
      <button
        className="flex w-full items-center justify-between px-4 py-3.5 text-left transition hover:bg-[#222832]"
        onClick={onToggle}
      >
        <div className="flex flex-col gap-1.5">
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-[#8B92A0]">
            <span>{lectureCount} lesson{lectureCount !== 1 ? "s" : ""}</span>
            {quizCount > 0 && <span>• {quizCount} quiz{quizCount !== 1 ? "zes" : ""}</span>}
          </div>
        </div>
        {isOpen ? (
          <ChevronDown className="h-4 w-4 text-[#8B92A0]" />
        ) : (
          <ChevronRight className="h-4 w-4 text-[#8B92A0]" />
        )}
      </button>
      {isOpen && (
        <div className="border-t border-[#222832]">
            ...
            return (
              ...
              >
                <div className={cn(
                  ...
                  isActive ? "bg-[#4ECDC4] text-[#0F1419]" : "bg-[#222832] text-[#9CA3AF]"
                )}>
                  {isActive ? (
                ...
                <div className="flex flex-1 flex-col gap-0.5 min-w-0">
                  <span className={cn(
                    ...
                    isActive ? "text-[#4ECDC4]" : "text-[#F8F9FA]"
                  )}>
...
export const ModuleSidebar = ({
  ...
  return (
    <aside className="h-full w-full space-y-4 overflow-y-auto pb-10 p-4">
      ...
      <div className="space-y-3 rounded-lg border border-[#222832] bg-[#1E2329] p-4 shadow-lg shadow-black/30">
        <div className="flex items-start gap-3">
          <BookOpenCheck className="mt-0.5 h-5 w-5 text-[#4ECDC4] flex-shrink-0" />
          <div className="flex-1 min-w-0">
            <p className="text-xs font-medium uppercase tracking-wide text-[#8B92A0]">Course Progress</p>
            <h3 className="mt-1 text-base font-semibold leading-tight text-[#F8F9FA] line-clamp-2">
              {courseTitle || "Course"}
            </h3>
          </div>
        </div>
        <div className="space-y-2 rounded-lg bg-[#0F1419] border border-[#222832] p-3">
          <div className="flex items-center justify-between text-xs">
            <span className="font-semibold text-[#4ECDC4]">{progress}%</span>
            <span className="text-[#8B92A0]">{progress}% complete</span>
          </div>
          <div className="h-2 overflow-hidden rounded-full bg-[#222832]">
            <div 
