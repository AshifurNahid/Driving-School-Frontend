...
export const QuizViewer = ({ quiz, courseId }: QuizViewerProps) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const serverMessage =
        (error as { response?: { data?: { status?: { message?: string }; message?: string; error?: string } } }).response?.data
          ?.status?.message ||
      ...
      toast({
        ...
        description: serverMessage ?? "We couldn't submit your quiz. Please try again.",
        variant: "destructive",
      });
    } finally {
