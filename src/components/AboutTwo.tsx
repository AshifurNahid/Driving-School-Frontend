...
export default function AboutTwo() {
  ...
  const cardSurface = isDarkMode ? 'bg-gray-800' : lightPalette.surface;
  const mutedText = isDarkMode ? 'text-gray-300' : lightPalette.secondary;
  const accentText = isDarkMode ? 'text-blue-200' : lightPalette.accent;
  const headlineText = isDarkMode ? 'text-white' : lightPalette.primary;
  ...
  const programs = [
    {
      ...
      icon: "🟦",
      border: isDarkMode ? "border-blue-400" : lightPalette.border,
      ring: isDarkMode ? "ring-blue-400" : "ring-slate-200",
      accentBg: isDarkMode ? "bg-blue-900" : lightPalette.softSurface,
      accentText: isDarkMode ? "text-blue-200" : lightPalette.primary
    },
    {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${isDarkMode ? 'bg-gray-900' : 'bg-gradient-to-br from-slate-50 via-gray-50 to-slate-100'}`}>
      {/* Hero Section */}
      <div className={`relative overflow-hidden ${isDarkMode ? 'bg-gradient-to-r from-gray-800 via-gray-700 to-gray-800' : 'bg-slate-50'} ${isDarkMode ? 'text-blue-200' : 'text-slate-900'}`}>
        ...
        <div className="relative max-w-7xl mx-auto px-6 py-20 sm:py-28">
          <div className="text-center">
            ...
            <p className={`text-lg ${isDarkMode ? 'text-blue-300' : 'text-slate-500'}`}>
              Across Newfoundland & Labrador
            </p>
          </div>
        </div>
        <div className={`absolute bottom-0 left-0 right-0 h-16 bg-gradient-to-t ${isDarkMode ? 'from-gray-900' : 'from-slate-50'} to-transparent`} />
      </div>

      {/* Who We Are Section */}
      <div className="max-w-6xl mx-auto px-6 py-16">
        <div className={`${cardSurface} rounded-2xl shadow-xl p-8 sm:p-12 border-t-4 ${isDarkMode ? 'border-blue-400' : lightPalette.border}`}>
          <h2 className={`text-3xl sm:text-4xl font-bold mb-6 flex items-center gap-3 ${headlineText}`}>
            <Shield className={`w-10 h-10 ${isDarkMode ? 'text-blue-400' : 'text-slate-800'}`} />
            Who We Are
          </h2>
      ...
      <div className={`${isDarkMode ? 'bg-gray-800' : lightPalette.softSurface} py-16`}>
        <div className="max-w-6xl mx-auto px-6">
          <h2 className={`text-3xl sm:text-4xl font-bold text-center mb-12 ${headlineText}`}>
            Our Philosophy
          </h2>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {philosophy.map((item, index) => (
              <div key={index} className={`${isDarkMode ? 'bg-gray-700 border-gray-600' : 'bg-white bg-opacity-60 backdrop-blur-lg border-slate-200'} rounded-xl p-8 border hover:bg-opacity-80 transition-all duration-300`}>
                <div className={`${accentText} mb-4`}>{item.icon}</div>
                <div className="flex items-center gap-2 mb-3">
                  <CheckCircle className="w-6 h-6 text-green-400" />
                  <h3 className={`text-2xl font-bold ${isDarkMode ? 'text-blue-200' : lightPalette.primary}`}>{item.title}</h3>
                </div>
                <p className={isDarkMode ? 'text-gray-300' : lightPalette.secondary}>{item.description}</p>
              </div>
            ))}
          </div>
          <p className={`text-lg text-center mt-12 max-w-4xl mx-auto ${mutedText}`}
          >
      ...
      {/* Programs Section */}
      <div className="max-w-6xl mx-auto px-6 py-16">
        <h2 className={`text-3xl sm:text-4xl font-bold text-center mb-4 ${headlineText}`}>Our Programs</h2>
        <p className={`text-lg text-center mb-12 ${mutedText}`}>
          ...
          <div className="grid sm:grid-cols-2 gap-6">
            {benefits.map((benefit, index) => (
              <div key={index} className={`${isDarkMode ? 'bg-gray-700' : 'bg-white'} rounded-xl p-6 shadow-md hover:shadow-lg transition-shadow duration-300 flex items-start gap-4`}>
                <div className={`${isDarkMode ? 'text-blue-400' : 'text-slate-800'} flex-shrink-0 mt-1`}>{benefit.icon}</div>
                <p className={isDarkMode ? 'text-gray-300' : lightPalette.secondary}>{benefit.text}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Commitment Section */}
      <div className="max-w-6xl mx-auto px-6 py-16">
        <div className={`${isDarkMode ? 'bg-gradient-to-r from-blue-900 to-gray-800' : 'bg-gradient-to-r from-slate-100 to-gray-200'} rounded-2xl shadow-2xl p-8 sm:p-12 ${isDarkMode ? 'text-blue-100' : lightPalette.primary}`}>
          ...
          <p className={`text-lg text-center font-semibold ${isDarkMode ? 'text-blue-100' : lightPalette.secondary}`}
          >
      ...
      {/* CTA Section */}
      <div className={`${isDarkMode ? 'bg-gradient-to-r from-blue-900 to-blue-800' : 'bg-slate-100'} py-16`}>
        <div className="max-w-4xl mx-auto px-6 text-center">
          <h2 className={`text-3xl sm:text-4xl font-bold mb-4 ${isDarkMode ? 'text-blue-100' : lightPalette.primary}`}>📍 Join Us Today</h2>
          <p className={`text-xl mb-8 ${isDarkMode ? 'text-blue-200' : lightPalette.secondary}`}
          >
            Start your journey toward confidence, safety, and independence on the road.
          </p>
          <p className={`text-lg mb-8 ${isDarkMode ? 'text-blue-200' : lightPalette.muted}`}
          >
          ...
          <Link to="/courses">
            <button className={`${isDarkMode ? 'bg-blue-800 text-blue-100 hover:bg-blue-700' : 'bg-white text-slate-900 hover:bg-slate-50 border border-slate-200'} px-8 py-4 rounded-full text-lg font-bold transition-colors duration-300 shadow-lg hover:shadow-xl transform hover:scale-105`}>
